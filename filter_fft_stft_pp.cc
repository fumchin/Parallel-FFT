...
int main(int argc, char* argv[]){
    ...
    stft_data *data = (stft_data*)malloc(num_threads * sizeof(stft_data));
    for (int thread_count = 0; thread_count < num_threads; thread_count++) {
        ...
        data[thread_count].numSamples = numSamples;
        data[thread_count].sample_rate = sampleRate;

        rc = pthread_create(&threads[thread_count], NULL, stft, (void*)&data[thread_count]);
    
        cout << "id: "<< thread_count << "done"<<endl;
    }
    // cout << "done1"<<endl;
    ...
    //     bitrp (real_part, imag_part, n_fft);
    ...
    //         }
    //     }
    //     // FFT(real_part, imag_part, n_fft);
    //     // low_pass_filter(real_part, imag_part, sampleRate, n_fft, 500);
    //     bitrp (real_part, imag_part, n_fft);
    ...
    //         imag_part [j] /= n_fft;
    //     }
    //     // IFFT(real_part, imag_part, n_fft);
...
void* stft(void* input){
    stft_data* input_data = (stft_data*)input;
    int id = input_data->id;
    int numSamples = input_data->numSamples;
    int sampleRate = input_data->sample_rate;
    int num_threads = input_data->nthreads;

    int max_frame_size = (numSamples / hop_size) - 1;

    int offset = 0;
    vector<float>* real_part = new vector<float>;
    vector<float>* imag_part = new vector<float>;
    real_part->resize(n_fft);
    imag_part->resize(n_fft);
    
    pthread_t threads_fft[num_threads];
    int ID[num_threads];
    fft_data *data = (fft_data*)malloc(num_threads * sizeof(fft_data));

    

    for(int frame_count=id; frame_count<max_frame_size; frame_count+=num_threads){
        // cout << "frame_count" << frame_count << endl;
        ...
        *real_part = {audioFile.samples[0].begin() + offset , audioFile.samples[0].begin() + offset + n_fft};
        ...
        for (int thread_count = 0; thread_count < num_threads; thread_count++) {
            ...
            data[thread_count].nthreads = num_threads;
            // data[thread_count].stage = stage;
            data[thread_count].real_part = real_part;
            data[thread_count].imag_part = imag_part;

            pthread_create(&threads_fft[thread_count], NULL, FFT, (void*)&data[thread_count]);
            // pthread_join(threads[thread_count], NULL);
        }
        for(int cpu_index=0; cpu_index<num_threads; cpu_index++){
            pthread_join(threads_fft[cpu_index], NULL);
        }
        // }
        // FFT(real_part, imag_part, n_fft);
        // low_pass_filter(real_part, imag_part, sampleRate, n_fft, 500);
        ...
        for (int thread_count = 0; thread_count < num_threads; thread_count++) {
            ...
            pthread_create(&threads_fft[thread_count], NULL, IFFT, (void*)&data[thread_count]);
            // pthread_join(threads[thread_count], NULL);
        }
        for(int cpu_index=0; cpu_index<num_threads; cpu_index++){
            pthread_join(threads_fft[cpu_index], NULL);
...
void* FFT(void* input)
{
    ...
    vector<float>* real_part = (data->real_part);
    vector<float>* imag_part = (data->imag_part);
    ...
    for (i = id; i < n_fft; i += nthreads){
        ...
        if ( b > i){
            ...
            // swap ((*imag_part) [i], (*imag_part) [b]);
            iter_swap((*real_part).begin()+i, (*real_part).begin()+b);
            iter_swap((*imag_part).begin()+i, (*imag_part).begin()+b);
        }
    }
    // pthread_barrier_wait(&barr);
    for (int m = 2; m <= n_fft; m *= 2){
        if(nthreads >= m){
            ...
            for (k = start_pos; k < end_pos; k += m){
                for (j = 0; j < m / 2; j ++){
                    ...
                    index2 = index1 + m / 2;
                    t = n_fft * j / m; 
                    treal = wreal [t] * (*real_part) [index2] - wimag [t] * (*imag_part) [index2];
                    timag = wreal [t] * (*imag_part) [index2] + wimag [t] * (*real_part) [index2];
                    ureal = (*real_part) [index1];
                    uimag = (*imag_part) [index1];
                    (*real_part) [index1] = ureal + treal;
                    (*imag_part) [index1] = uimag + timag;
                    (*real_part) [index2] = ureal - treal;
                    (*imag_part) [index2] = uimag - timag;
                }
            }
        }else{
        ...
        bool pbw = pthread_barrier_wait(&barr_fft);
        cout << pbw << endl;
    }
    pthread_exit(NULL);
    return NULL;
        ...
        if(nthreads >= m){
            ...
            for (k = start_pos; k < end_pos; k += m){
                for (j = 0; j < m / 2; j ++){
                    ...
                    index2 = index1 + m / 2;
                    t = n_fft * j / m; 
                    treal = wreal_i [t] * (*real_part) [index2] - wimag_i [t] * (*imag_part) [index2];
                    timag = wreal_i [t] * (*imag_part) [index2] + wimag_i [t] * (*real_part) [index2];
                    ureal = (*real_part) [index1];
                    uimag = (*imag_part) [index1];
                    (*real_part) [index1] = ureal + treal;
                    (*imag_part) [index1] = uimag + timag;
